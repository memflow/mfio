...
// From mio
pub(crate) fn new_for_addr(address: SocketAddr) -> std::io::Result<(AddressFamily, RawFd)> {
    let domain = match address {
        SocketAddr::V4(_) => AddressFamily::Inet,
        SocketAddr::V6(_) => AddressFamily::Inet6,
    };
    new_socket(domain, SockType::Stream).map(|v| (domain, v))
}

/// Create a new non-blocking socket.
pub(crate) fn new_socket(domain: AddressFamily, socket_type: SockType) -> std::io::Result<RawFd> {
    #[cfg(any(
        target_os = "android",
        target_os = "dragonfly",
        target_os = "freebsd",
    ...
    let flags = SockFlag::SOCK_CLOEXEC.union(SockFlag::from_bits_truncate(libc::SOCK_NONBLOCK));
    #[cfg(not(any(
        target_os = "android",
        target_os = "dragonfly",
        target_os = "freebsd",
